    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header">
                <h4 class="mb-0">
                    <i class="fas fa-clipboard-edit"></i> Editar Inscrição
                </h4>
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="vaqueiro_id" id="vaqueiro_id" class="form-select @error('vaqueiro_id') is-invalid @enderror" required>
                                ...
                                @foreach($competidores as $competidor)
                                    <option value="{{ $competidor->id }}" {{ old('vaqueiro_id', $inscricao->vaqueiro_id) == $competidor->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="bate_esteira_id" id="bate_esteira_id" class="form-select @error('bate_esteira_id') is-invalid @enderror" required>
                                ...
                                @foreach($competidores as $competidor)
                                    <option value="{{ $competidor->id }}" {{ old('bate_esteira_id', $inscricao->bate_esteira_id) == $competidor->id ? 'selected' : '' }}>
                                        {{ $competidor->nome }} - {{ $competidor->representacao }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="forma_pagamento" id="forma_pagamento" class="form-select @error('forma_pagamento') is-invalid @enderror" required>
                                ...
                                <option value="Dinheiro" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Dinheiro' ? 'selected' : '' }}>Dinheiro</option>
                                <option value="Pix" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Pix' ? 'selected' : '' }}>Pix Manual</option>
                                @php $gateway = \App\Models\Setting::getValue('payment.gateway', 'none') @endphp
                                @if($gateway !== 'none' || $inscricao->forma_pagamento == 'Pix (Gateway)')
                                    <option value="Pix (Gateway)" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Pix (Gateway)' ? 'selected' : '' }} class="fw-bold text-success">Pix Online (Imediato)</option>
                                @endif
                                <option value="Cartão" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Cartão' ? 'selected' : '' }}>Cartão Manual</option>
                                <option value="Crediário" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Crediário' ? 'selected' : '' }}>Crediário</option>
                                <option value="Cheque" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Cheque' ? 'selected' : '' }}>Cheque</option>
                                <option value="Troca" {{ old('forma_pagamento', $inscricao->forma_pagamento) == 'Troca' ? 'selected' : '' }}>Troca</option>
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="valor_total" id="valor_total" class="form-control @error('valor_total') is-invalid @enderror"
                                   value="{{ old('valor_total', $inscricao->valor_total) }}" step="0.01" min="0" placeholder="0,00" required>
                            @error('valor_total')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status_pagamento" id="status_pagamento" class="form-select @error('status_pagamento') is-invalid @enderror" required>
                                ...
                                <option value="pago" {{ old('status_pagamento', $inscricao->status_pagamento) == 'pago' ? 'selected' : '' }}>Pago</option>
                                <option value="cancelado" {{ old('status_pagamento', $inscricao->status_pagamento) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                            </select>
